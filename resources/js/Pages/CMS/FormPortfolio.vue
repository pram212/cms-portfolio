...
const form = useForm({
    ...
    images: props.portfolio ? JSON.parse(props.portfolio.images).map(item => ({ path: item, temp: null }) ) : [],
    modules: props.portfolio ? JSON.parse(props.portfolio.modules).map((item) => item ) : [],
    description: props.portfolio?.description,
    demo: props.portfolio?.demo,
})

const submit = () => {
    if (props.portfolio) router.post(route('cms.portfolios.update', props.portfolio.id), { _method: 'put', form })
    else form.post(route('cms.portfolios.store'), { preserveState: true })
}

// technologies handles
...
const previewNewImage = (event) => {
    ...
    reader.onload = () => {
        ...
        form.images.push({path: null, file: file, temp: newImagePreview })
    };
                ...
                <div class="bg-base-100 overflow-hidden shadow-sm sm:rounded-lg p-10">
                    ...
                    <form @submit.prevent="submit">
                        <label class="input input-bordered flex items-center gap-2 mb-3 capitalize font-semibold"
                            :class="{ 'input-error': form.errors.project_title }">
                            Project title :
                            <input type="text" class="grow" v-model="form.project_title" />
                            <small class="text-xs text-error" v-if="form.errors.project_title">{{ form.errors.project_title }}</small>
                        </label>

                        <div class="flex justify-between space-x-2">
                            <label :class="{ 'input-error': form.errors.start, 'input input-bordered w-full flex items-center gap-2 mb-3 capitalize font-semibold' : true }">
                                Start :
                                <input type="date" class="grow" v-model="form.start" />
                                <small v-if="form.errors.start">{{ form.errors.start }}</small>
                            </label>
                            <label :class="{ 'input-error': form.errors.start, 'input input-bordered w-full flex items-center gap-2 mb-3 capitalize font-semibold' : true }">
                                End :
                                <input type="date" class="grow" v-model="form.end" />
                                <small v-if="form.errors.end">{{ form.errors.end }}</small>
                            </label>
                        </div>
                        ...
                        <label class="form-control font-semibold">
                            <div class="label">
                                ...
                                <span class="label-text-alt text-error text-xs" v-if="form.errors.description">{{
                        ...
                        <div class="flex justify-between space-x-1">
                            <!-- Technologies form -->
                            <div class="overflow-x-auto border my-2 shadow-md w-1/2 py-2">
                                <table class="table table-xs">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, i) in form.technologies" :key="i" class="uppercase">
                                            <th>{{ i + 1 }}</th>
                                            <td><input type="text" class="input w-full input-sm input-borered input-secondary uppercase" v-model="form.technologies[i]"></td>
                                            <td class="text-right">
                                                <button type="button" @click="removeTech(i)" class="btn btn-sm btn-error">
                                        ...
                                        <tr>
                                            <td class="text-center" colspan="3">
                                                <div class="join w-full">
                                                    <input class="input w-full input-bordered join-item input-sm uppercase" placeholder="" v-model="techVal"/>
                                                    <button type="button" class="btn join-item btn-sm btn-success" @click="addTech">Add</button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tfoot>
                                ...
                                <table class="table table-xs">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, i) in form.modules" :key="i" class="uppercase">
                                            <th>{{ i + 1 }}</th>
                                            <td><input type="text" class="input w-full input-sm input-borered input-secondary uppercase" v-model="form.modules[i]"></td>
                                            <td class="text-right">
                                                <button type="button" @click="removeModule(i)" class="btn btn-sm btn-error">
                                                    <i class="fa fa-times" aria-hidden="true"></i>
                                                </button>
                                            </td>
                                        </tr>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td class="text-center" colspan="3">
                                                <div class="join w-full">
                                                    <input type="text" class="input w-full input-bordered join-item input-sm uppercase" placeholder="" v-model="modulVal"/>
                                                    <button type="button" class="btn join-item btn-sm btn-success" @click="addModule">Add</button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tfoot>
                        ...
                        <div class="border shadow-md p-3">
                            ...
                            <div class="grid grid-cols-6 my-2">
                                <div class="avatar" v-for="(image, index) in form.images" :key="index">
                                    <div class="w-36 rounded-xl shadow-md">
                                        <button type="button" @click="removeImage(index)" class="absolute top-[-7px] right-3 bg-error btn btn-sm rounded-full">
                                            <i class="fa fa-times" aria-hidden="true"></i>
                                        </button>
